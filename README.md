...
| GET | `/events/:id/shoots/credentials` | owner or collaborator | `{ connected: false }`, or `{ connected: true, ftp_host, ftp_port, ftp_username, ftp_password }` if PandaShoots (camera-to-cloud live upload) is already set up — see "PandaShoots" below. |
| POST | `/events/:id/shoots/credentials` | owner or collaborator · 20/hour/IP | Generates fresh PandaShoots credentials (first setup, or a "Regenerate" — this simply overwrites the previous ones, which stop working immediately). Same response shape as the `GET` above. |
| DELETE | `/events/:id/shoots/credentials` | owner or collaborator | Revokes PandaShoots access — the camera's saved FTP settings stop authenticating. Doesn't touch photos already ingested. `204` on success. |
| GET | `/events/:id/live/stream` | owner or collaborator | Server-Sent Events stream of `photo_added`/`photo_skipped` events as PandaShoots ingests camera captures — lets an open event page update its gallery live during a shoot. Purely additive; the gallery still loads fine without ever opening this stream. |
| POST | `/events/:id/drive-backup/toggle` | owner or collaborator | **Advanced/beta** — see "Drive backup" below. `{ enabled }` → turns on/off mirroring PandaShoots captures into this event's connected Drive folder using the platform's single Drive account. `400` if no Drive folder is connected yet or that platform account isn't configured; `403` if the event's owner isn't on `DRIVE_BACKUP_BETA_EMAILS`. |
...
## PandaShoots (camera-to-cloud live upload)
...
3. `src/lib/ftpShoots.js` runs an in-process FTP server (`ftp-srv`). On login,
   ...
   (`storage/shoots-incoming/<eventId>/`) — a camera can only ever write into
...
which is unrelated to PandaShoots itself). **Not yet verified against a real
...
manually. This is the natural follow-up to PandaShoots: photos can now keep landing
...
`lib/driveSync.js`) and after each single PandaShoots capture
...
burst of PandaShoots captures during a shoot produces one ping, not a flood.
...
connected once (`GOOGLE_DRIVE_BACKUP_REFRESH_TOKEN`), and every event's PandaShoots
...
account itself isn't configured. When on, each PandaShoots capture
...
- **~6 hours after the last capture** (inferred "this shoot looks
  finished" from `Event.lastShootsCaptureAt` going quiet) — a one-time email
...
APIs but needs a real `GOOGLE_CLIENT_SECRET` + refresh token and a real PandaShoots
